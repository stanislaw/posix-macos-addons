...
int mac_sem2_destroy(mac_sem2_t *psem) {
  dispatch_barrier_sync(sync_queue(), ^{
    ...
    if (count == SEMAPHORE_ALREADY_RELEASED) {
      ...
      printf("warning: mac_sem2_destroy: the unbalanced calls to wait/post is "
...
int mac_sem2_post(mac_sem2_t *psem) {
  dispatch_sync(sync_queue(), ^{
    ...
    if (count == SEMAPHORE_ALREADY_RELEASED) {
      printf(
        "warning: mac_sem2_post: post on a released semaphore is detected\n");
      return;
    }
...
int mac_sem2_trywait(mac_sem2_t *psem) {
  ...
  dispatch_sync(sync_queue(), ^{
    uint64_t count = get_count(psem);
    if (count == SEMAPHORE_ALREADY_RELEASED) {
      printf("warning: mac_sem2_trywait: wait on a released semaphore is "
             "detected\n");
      result = 0;
      return;
    }
...
int mac_sem2_wait(mac_sem2_t *psem) {
  ...
  dispatch_sync(sync_queue(), ^{
    ...
    if (count == SEMAPHORE_ALREADY_RELEASED) {
      printf(
        "warning: mac_sem2_wait: wait on a released semaphore is detected\n");
      result = 0;
      return;
    }
    increment_count(psem);
...
int mac_sem2_timedwait(mac_sem2_t *psem, const struct timespec *abstim) {
  ...
  dispatch_sync(sync_queue(), ^{
    uint64_t count = get_count(psem);
    if (count == SEMAPHORE_ALREADY_RELEASED) {
      printf("warning: mac_sem2_timedwait: wait on a released semaphore is "
             "detected\n");
      result = 0;
      return;
    }
